...
{
  GMACRXDESCRIPTOR  atRxDescriptors[ ATMELSAM4IF_NUM_RX_BUFFERS ];
  GMACTXDESCRIPTOR  atTxDescriptors[ ATMELSAM4IF_NUM_TX_BUFFERS ];
  struct pbuf*      aptRxBuffers[ ATMELSAM4IF_NUM_RX_BUFFERS ];
  U8                anTxBuffers[ ATMELSAM4IF_NUM_TX_BUFFERS ][ GMAC_TX_UNITSIZE ];
  U32               uRxIndex;
  U32               uTxIndex;
...
static  U8            anMacAddress[ ATMELSAM4IF_MACADDR_SIZE ];
 ...
 *
 * @return      TRUE - always flush event
 *
 *****************************************************************************/
BOOL EthernetHandler_ProcessInput( TASKARG xArg )
{
...
static void LowLevelInitialize( struct netif *ptNetIf )
{
  ...
  {
    // set auto negotiate
    if ( KSZ8051MNLPhy_AutoNegotiate(( PU32 )GMAC, 0 ) != GMAC_STATUS_OK )
    {
      // report error
    ...
    {
      // establish link
      while( KSZ8051MNLPhy_SetLink(( PU32 )GMAC, 0, 1 ) != GMAC_STATUS_OK )
      {
        // report error
...
static err_t LowLevelOutput( struct netif* ptNetIf, struct pbuf* ptBuffer )
{
  ...
  ptDevice->uTxIndex = ( ptDevice->uTxIndex + 1 ) % ATMELSAM4IF_NUM_TX_BUFFERS;
...
static struct pbuf* LowLevelInput( struct netif* ptNetIf )
{
  ...
  if ( Gmac_GetRxStatus( GMAC ) & GMAC_RX_ERROR_MASK )
  {
    ...
    // free all RX buffers
    for( uIndex = 0; uIndex < ATMELSAM4IF_NUM_RX_BUFFERS; uIndex++ )
    {
      // check for not empty
  ...
  if (( ptRxDescriptor->tAddr.uValue & GMAC_RXD_OWNERSHIP ) == GMAC_RXD_OWNERSHIP )
  {
    ...
    ptDevice->uRxIndex = ( ptDevice->uRxIndex + 1 ) % ATMELSAM4IF_NUM_RX_BUFFERS;
...
static void TxInitialize( PGMACDEVICE ptDevice )
{
  ...
  for ( uIndex = 0; uIndex < ATMELSAM4IF_NUM_TX_BUFFERS; uIndex++ )
  {
...
static void RxPopulateQueue( PGMACDEVICE ptDevice )
{
  ...
  for ( uIndex = 0; uIndex < ATMELSAM4IF_NUM_RX_BUFFERS; uIndex++ )
  {
    ...
    if ( ptDevice->aptRxBuffers[ uIndex ] == NULL )
    {
      ...
      if ( uIndex == ( ATMELSAM4IF_NUM_RX_BUFFERS - 1 ))
      {
...
//    TaskManager_PostEvent( ATMELSAM4IF_PROCESS_RCV_TASK_ENUM, 0 );
//  }
}
