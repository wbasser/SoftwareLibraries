...
#define USBCDC_NUM_STRING_DESCRIPTORS ( sizeof( apUSBCDCStringDescriptors ) /  sizeof( PU8 ))
...
void StellarisUSBCDCHandler_Initialize( void )
{
  ...
  tCdcDevice.pfnTxCallback			    = USBBufferEventCallback;
	...
	tCdcDevice.ppStringDescriptors		= (const unsigned char * const *)&apUSBCDCStringDescriptors;
  ...
  U8  nData;
	...
	switch( uEvent )
	{
		case USB_EVENT_RX_AVAILABLE :
			...
			{
				// post it
				#if ( StellarisUSBCDCHandler_RCVCHR_TASK != -1 )
...
static U32 CtrlHandler( PVOID pvCBData, U32 uEvent, U32 uMsgValue, PVOID pvMsgData )
{
  U32 uStatus = 0;
	...
	switch( uEvent )
	{
      ...
      #if ( StellarisUSBCDCHandler_USBCON_LED != -1 )
